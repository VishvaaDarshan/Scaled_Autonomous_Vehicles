             ...
             for batch_sample in batch_samples:
                 if batch_sample[5] != "filename":
                     ...
                     if aug:
                       ...
                       flip_speed = speed
                       # print(flip_speed,flip_angle)
                       choice = [flip_angle, flip_speed]
                       images.append(flip_image)
                       batch_ch.append(choice)

             X_train = np.array(images)
             y_train = np.array(batch_ch)
...
validation_generator = generator(validation_samples, batch_size=batch_size_value, aug=1)
...
checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=2, save_best_only=True, mode='auto', period=1)
