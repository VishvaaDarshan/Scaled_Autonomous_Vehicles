...
class cmd_vel_node(object):
    def __init__(self):
      ...
      """ Variables """
      self.model_path = '/home/prajval/end_to_end_learning/src/diy_driverless_car_ROS-master/rover_ml/output/weights/model.h5'
      self.cmdvel = Twist()
    ...
    def run(self):
         ...
         f = h5py.File('/home/prajval/end_to_end_learning/src/diy_driverless_car_ROS-master/rover_ml/output/weights/model.h5', mode='r')
         ...
         with open('/home/prajval/end_to_end_learning/src/diy_driverless_car_ROS-master/rover_ml/output/weights/model.json', 'r') as f:
             model = model_from_json(f.read())

         model = load_model('/home/prajval/end_to_end_learning/src/diy_driverless_car_ROS-master/rover_ml/output/weights/model.h5')
         ...
         while True:
             ...
             if self.imgRcvd:
                 ...
                 # self.crop_img = self.resized_image[75:180,0:320]
                 
                 # step 2: 
                 image_array = np.asarray(self.resized_image)
                 
                 # step 3: 
                 # print((model.predict(image_array[None, :, :, :], batch_size=1))[0][0])
                 self.angle = (model.predict(image_array[None, :, :, :], batch_size=1))[0][0]
                 self.angle = self.angle*2.75
                 print("angle",self.angle)
                 # self.angle = -1.57 if self.angle < -1.57 else 1.57 if self.angle > 1.57 else self.angle
                 self.speed = abs(model.predict(image_array[None, :, :, :], batch_size=1))[0][1]

                 self.speed = (self.speed)
                 print("Speed",self.speed)

                 # self.speed  = 0.0 if self.speed < 0 else 2.5 if self.speed > 2.5 else self.speed
                 self.cmdvel.angular.z = self.angle

                 self.cmdvel.linear.x = self.speed
